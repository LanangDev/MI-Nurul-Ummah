...
<title><?= html_escape($judul) ?> - <?= html_escape($kontak->nama_sekolah ?? 'Madrasah Ibtidaiyah') ?></title>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .guru-card .thumb{width:100%;height:190px;object-fit:cover;}
  .guru-card .thumb-placeholder{width:100%;height:190px;display:flex;align-items:center;justify-content:center;
...
<section class="page-hero">
  <div class="container">
    <div class="breadcrumb">
      <a href="<?= base_url() ?>">Beranda</a> / Data Guru
    </div>
    <span class="kicker">Tenaga pendidik</span>
    <h1><?= html_escape($judul) ?></h1>
    <p>Daftar tenaga pendidik dan kependidikan yang mendampingi siswa dalam proses belajar mengajar.</p>
  </div>
</section>
    ...
    <div class="guru-grid">
      <?php if (!empty($guru)): ?>
        <?php foreach ($guru as $g): ?>
          <a href="<?= base_url('Data_guru/detail/' . $g->id_guru) ?>" class="guru-card">
            <?php if (!empty($g->foto) && file_exists('./upload/foto_guru/' . $g->foto)): ?>
              <img class="thumb" src="<?= base_url('upload/foto_guru/' . $g->foto) ?>" alt="<?= html_escape($g->nama_guru) ?>">
            <?php else: ?>
              <div class="thumb-placeholder"><?= html_escape(strtoupper(substr($g->nama_guru, 0, 1))) ?></div>
            <?php endif; ?>
            <div class="body">
              <h3><?= html_escape($g->nama_guru) ?></h3>
              <div class="jabatan"><?= html_escape($g->jabatan ?? '-') ?></div>
              <div class="meta">
                <?php if (!empty($g->mata_pelajaran)): ?><span><i class="bi bi-journal-bookmark"></i> <?= html_escape($g->mata_pelajaran) ?></span><?php endif; ?>
                <?php if (!empty($g->status)): ?><span><i class="bi bi-tag"></i> <?= html_escape($g->status) ?></span><?php endif; ?>
              </div>
        ...
        <?php endforeach; ?>
      <?php else: ?>
        <p class="empty-state">Belum ada data guru.</p>
      <?php endif; ?>
    </div>
