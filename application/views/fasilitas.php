...
<title>Fasilitas Sekolah - <?= html_escape($kontak->nama_sekolah ?? 'Madrasah Ibtidaiyah') ?></title>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .status-baik{color:var(--forest);}
  .status-ringan{color:var(--brass);}
  .status-berat{color:var(--clove);}
...
<section class="page-hero">
  <div class="container">
    <div class="breadcrumb"><a href="<?= base_url() ?>">Beranda</a> / Fasilitas</div>
    <span class="kicker">Sarana &amp; prasarana</span>
    <h1>Fasilitas Sekolah</h1>
    <p>Daftar fasilitas yang dimiliki <?= html_escape($kontak->nama_sekolah ?? 'Madrasah Ibtidaiyah') ?> beserta kondisi terkini.</p>
  </div>
</section>
    ...
    <div class="guru-grid">
      <?php if (!empty($fasilitas)): ?>
        <?php foreach ($fasilitas as $fas): ?>
          <?php
            $kondisi = trim($fas->kondisi ?? '');
            $kondisiClass = 'status-baik';
            if (strtolower($kondisi) === 'rusak ringan') $kondisiClass = 'status-ringan';
            if (strtolower($kondisi) === 'rusak berat') $kondisiClass = 'status-berat';
          ?>
          <a href="<?= base_url('Fasilitas/detail/' . $fas->id_fasilitas) ?>" class="guru-card">
            <?php if (!empty($fas->foto) && file_exists('./upload/foto_fasilitas/' . $fas->foto)): ?>
              <img class="thumb" src="<?= base_url('upload/foto_fasilitas/' . $fas->foto) ?>" alt="<?= html_escape($fas->nama_fasilitas) ?>">
            <?php else: ?>
              <div class="thumb-placeholder"><i class="bi bi-building"></i></div>
            <?php endif; ?>
            <div class="body">
              <h3><?= html_escape($fas->nama_fasilitas) ?></h3>
              <div class="jabatan"><?= !empty($fas->jumlah) ? html_escape($fas->jumlah) . ' unit' : '-' ?></div>
              <div class="meta">
                <?php if (!empty($kondisi)): ?>
                  <span class="status-badge <?= $kondisiClass ?>"><i class="bi bi-circle-fill" style="font-size:8px;"></i> <?= html_escape($kondisi) ?></span>
                <?php endif; ?>
              </div>
