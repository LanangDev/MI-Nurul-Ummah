...
<title>Ekstrakurikuler - <?= html_escape($kontak->nama_sekolah ?? 'Madrasah Ibtidaiyah') ?></title>
    ...
    <div class="guru-grid">
      <?php if (!empty($ekstrakurikuler)): ?>
        <?php foreach ($ekstrakurikuler as $ek): ?>
          <?php $isAktif = (strtolower(trim($ek->status ?? '')) === 'aktif'); ?>
          <a href="<?= base_url('ekstrakurikuler/detail/' . $ek->id_ekstrakurikuler) ?>" class="guru-card">
            <?php if (!empty($ek->foto) && file_exists('./upload/foto_ekstra/' . $ek->foto)): ?>
              <img class="thumb" src="<?= base_url('upload/foto_ekstra/' . $ek->foto) ?>" alt="<?= html_escape($ek->nama_ekstrakurikuler) ?>">
            <?php else: ?>
              <div class="thumb-placeholder"><?= html_escape(strtoupper(substr($ek->nama_ekstrakurikuler, 0, 1))) ?></div>
            <?php endif; ?>
            <div class="body">
              <h3><?= html_escape($ek->nama_ekstrakurikuler) ?></h3>
              <div class="jabatan"><?= html_escape($ek->pembina ?? '-') ?></div>
              <div class="meta">
                <?php if (!empty($ek->waktu)): ?><span><i class="bi bi-clock"></i> <?= html_escape($ek->waktu) ?></span><?php endif; ?>
                <?php if ($isAktif): ?>
                  <span class="status-badge status-buka"><i class="bi bi-circle-fill" style="font-size:8px;"></i> Aktif</span>
                <?php else: ?>
                  <span class="status-badge status-tutup"><i class="bi bi-circle-fill" style="font-size:8px;"></i> Tidak Aktif</span>
                <?php endif; ?>
              </div>
