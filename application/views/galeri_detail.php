...
<title><?= html_escape($galeri->judul_kegiatan) ?> - <?= html_escape($kontak->nama_sekolah ?? 'Madrasah Ibtidaiyah') ?></title>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .tag.lokasi{background:var(--forest-soft);color:var(--forest);}
  .tag.tanggal{background:#fbf1de;color:var(--brass);}
...
<section class="page-hero">
  <div class="container">
    <div class="breadcrumb">
      ...
      <span class="sep">/</span>
      <span class="current"><?= html_escape($galeri->judul_kegiatan) ?></span>
    </div>

    <div class="profile-card">
      <?php if (!empty($galeri->foto) && file_exists('./upload/galeri/' . $galeri->foto)): ?>
        <img class="profile-photo" src="<?= base_url('upload/galeri/' . $galeri->foto) ?>" alt="<?= html_escape($galeri->judul_kegiatan) ?>">
      <?php else: ?>
        <div class="profile-photo-placeholder"><i class="bi bi-images"></i></div>
      <?php endif; ?>

      <div class="profile-main">
        <h1><?= html_escape($galeri->judul_kegiatan) ?></h1>
        <div class="profile-tags">
          <?php if (!empty($galeri->lokasi)): ?>
            <span class="tag lokasi"><i class="bi bi-geo-alt-fill"></i> <?= html_escape($galeri->lokasi) ?></span>
          <?php endif; ?>
          <?php if (!empty($galeri->tanggal)): ?>
            <span class="tag tanggal"><i class="bi bi-calendar-fill"></i> <?= html_escape(date('d M Y', strtotime($galeri->tanggal))) ?></span>
          <?php endif; ?>
        </div>
      </div>
    </div>
  ...
  <div class="container">
    <div class="content-wrap">
      ...
      <div class="article-body">
        <div class="panel">
          <h4><i class="bi bi-clipboard2-data-fill"></i> Data Kegiatan</h4>
          <div class="info-grid">
            <div class="info-item">
              <label>Judul Kegiatan</label>
              <div class="val <?= empty($galeri->judul_kegiatan) ? 'empty' : '' ?>"><?= !empty($galeri->judul_kegiatan) ? html_escape($galeri->judul_kegiatan) : 'Belum diisi' ?></div>
            </div>
            <div class="info-item">
              <label>Tanggal</label>
              <div class="val <?= empty($galeri->tanggal) ? 'empty' : '' ?>"><?= !empty($galeri->tanggal) ? html_escape(date('d M Y', strtotime($galeri->tanggal))) : 'Belum diisi' ?></div>
            </div>
            <div class="info-item">
              <label>Lokasi</label>
              <div class="val <?= empty($galeri->lokasi) ? 'empty' : '' ?>"><?= !empty($galeri->lokasi) ? html_escape($galeri->lokasi) : 'Belum diisi' ?></div>
            </div>
            <div class="info-item" style="grid-column:1/-1;">
              <label>Deskripsi</label>
              <div class="val <?= empty($galeri->deskripsi) ? 'empty' : '' ?>"><?= !empty($galeri->deskripsi) ? html_escape($galeri->deskripsi) : 'Belum diisi' ?></div>
            </div>
          </div>
        </div>

        <a href="<?= base_url('galeri') ?>" class="back-link"><i class="bi bi-arrow-left"></i> Kembali ke Galeri kegiatan</a>
      </div>

      <aside class="sidebar">
        <h4><i class="bi bi-images"></i> Kegiatan Lainnya</h4>
        <?php if (!empty($galeri_lain)): ?>
          <?php foreach ($galeri_lain as $lain): ?>
            <a href="<?= base_url('galeri/detail/' . $lain->id_galeri) ?>" class="side-card">
              <?php if (!empty($lain->foto) && file_exists('./upload/galeri/' . $lain->foto)): ?>
                <img class="thumb" src="<?= base_url('upload/galeri/' . $lain->foto) ?>" alt="<?= html_escape($lain->judul_kegiatan) ?>">
              <?php else: ?>
                <div class="thumb-placeholder"><i class="bi bi-images"></i></div>
              <?php endif; ?>
              <div class="info">
                <h5><?= html_escape($lain->judul_kegiatan) ?></h5>
                <?php if (!empty($lain->tanggal)): ?><span><?= html_escape(date('d M Y', strtotime($lain->tanggal))) ?></span><?php endif; ?>
              </div>
            </a>
          <?php endforeach; ?>
        <?php else: ?>
          <p class="empty-note">Belum ada kegiatan lain.</p>
        <?php endif; ?>
      </aside>
