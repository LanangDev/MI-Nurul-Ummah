...
<title><?= html_escape($ekstrakurikuler->nama_ekstrakurikuler) ?> - <?= html_escape($kontak->nama_sekolah ?? 'Madrasah Ibtidaiyah') ?></title>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .tag.pembina{background:var(--forest-soft);color:var(--forest);}
...
<section class="page-hero">
  <div class="container">
    <div class="breadcrumb">
      ...
      <span class="sep">/</span>
      <a href="<?= base_url('ekstrakurikuler') ?>">ekstrakurikuler</a>
      <span class="sep">/</span>
      <span class="current"><?= html_escape($ekstrakurikuler->nama_ekstrakurikuler) ?></span>
    </div>

    <div class="profile-card">
      <?php if (!empty($ekstrakurikuler->foto) && file_exists('./upload/foto_ekstra/' . $ekstrakurikuler->foto)): ?>
        <img class="profile-photo" src="<?= base_url('upload/foto_ekstra/' . $ekstrakurikuler->foto) ?>" alt="<?= html_escape($ekstrakurikuler->nama_ekstrakurikuler) ?>">
      <?php else: ?>
        <div class="profile-photo-placeholder"><?= html_escape(strtoupper(substr($ekstrakurikuler->nama_ekstrakurikuler, 0, 1))) ?></div>
      <?php endif; ?>

      <div class="profile-main">
        <h1><?= html_escape($ekstrakurikuler->nama_ekstrakurikuler) ?></h1>
        <div class="profile-tags">
          <?php if (!empty($ekstrakurikuler->pembina)): ?>
            <span class="tag pembina"><i class="bi bi-briefcase-fill"></i> <?= html_escape($ekstrakurikuler->pembina) ?></span>
          <?php endif; ?>
          <?php if (!empty($ekstrakurikuler->hari)): ?>
            <span class="tag mapel"><i class="bi bi-journal-bookmark-fill"></i> <?= html_escape($ekstrakurikuler->hari) ?></span>
          <?php endif; ?>
          <?php if (!empty($ekstrakurikuler->status)): ?>
            <span class="tag status"><i class="bi bi-patch-check-fill"></i> <?= html_escape($ekstrakurikuler->status) ?></span>
          <?php endif; ?>
        </div>
      </div>
    </div>
        ...
        <div class="panel">
          ...
          <div class="info-grid">
            <div class="info-item">
              ...
              <div class="val <?= empty($ekstrakurikuler->nama_ekstrakurikuler) ? 'empty' : '' ?>"><?= !empty($ekstrakurikuler->nama_ekstrakurikuler) ? html_escape($ekstrakurikuler->nama_ekstrakurikuler) : 'Belum diisi' ?></div>
            </div>
            <div class="info-item">
              <label>Pembina</label>
              <div class="val <?= empty($ekstrakurikuler->pembina) ? 'empty' : '' ?>"><?= !empty($ekstrakurikuler->pembina) ? html_escape($ekstrakurikuler->pembina) : 'Belum diisi' ?></div>
            </div>
            <div class="info-item">
              <label>Hari</label>
              <div class="val <?= empty($ekstrakurikuler->hari) ? 'empty' : '' ?>"><?= !empty($ekstrakurikuler->hari) ? html_escape($ekstrakurikuler->hari) : 'Belum diisi' ?></div>
            </div>
            <div class="info-item">
              <label>Tempat</label>
              <div class="val <?= empty($ekstrakurikuler->tempat) ? 'empty' : '' ?>"><?= !empty($ekstrakurikuler->tempat) ? html_escape($ekstrakurikuler->tempat) : 'Belum diisi' ?></div>
            </div>
            <div class="info-item">
              <label>Waktu</label>
              <div class="val <?= empty($ekstrakurikuler->waktu) ? 'empty' : '' ?>"><?= !empty($ekstrakurikuler->waktu) ? html_escape($ekstrakurikuler->waktu) : 'Belum diisi' ?></div>
            </div>
            <div class="info-item">
              <label>Keterangan</label>
              <div class="val <?= empty($ekstrakurikuler->keterangan) ? 'empty' : '' ?>"><?= !empty($ekstrakurikuler->keterangan) ? html_escape($ekstrakurikuler->keterangan) : 'Belum diisi' ?></div>
            </div>
          </div>
        </div>

        <a href="<?= base_url('ekstrakurikuler') ?>" class="back-link"><i class="bi bi-arrow-left"></i> Kembali ke Daftar ekstrakurikuler</a>
      </div>

      <aside class="sidebar">
        <h4><i class="bi bi-people-fill"></i> ekstrakurikuler Lainnya</h4>
        <?php if (!empty($ekstrakurikuler_lain)): ?>
          <?php foreach ($ekstrakurikuler_lain as $lain): ?>
            <a href="<?= base_url('ekstrakurikuler/detail/' . $lain->id_ekstrakurikuler) ?>" class="side-card">
              <?php if (!empty($lain->foto) && file_exists('./upload/foto_ekstra/' . $lain->foto)): ?>
                <img class="thumb" src="<?= base_url('upload/foto_ekstra/' . $lain->foto) ?>" alt="<?= html_escape($lain->nama_ekstrakurikuler) ?>">
              <?php else: ?>
                <div class="thumb-placeholder"><?= html_escape(strtoupper(substr($lain->nama_ekstrakurikuler, 0, 1))) ?></div>
              <?php endif; ?>
              <div class="info">
                <h5><?= html_escape($lain->nama_ekstrakurikuler) ?></h5>
                <?php if (!empty($lain->pembina)): ?><span><?= html_escape($lain->pembina) ?></span><?php endif; ?>
              </div>
            </a>
          <?php endforeach; ?>
        <?php else: ?>
          <p class="empty-note">Belum ada ekstrakurikuler lain.</p>
        <?php endif; ?>
      </aside>
