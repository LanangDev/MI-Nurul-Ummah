...
<title>Galeri Kegiatan - <?= html_escape($kontak->nama_sekolah ?? 'Madrasah Ibtidaiyah') ?></title>
...
<section class="page-hero">
  <div class="container">
    <div class="breadcrumb"><a href="<?= base_url() ?>">Beranda</a> / Galeri</div>
    <span class="kicker">Dokumentasi sekolah</span>
    <h1>Galeri Kegiatan</h1>
    <p>Kumpulan dokumentasi kegiatan <?= html_escape($kontak->nama_sekolah ?? 'Madrasah Ibtidaiyah') ?> dari waktu ke waktu.</p>
  </div>
</section>
    ...
    <div class="guru-grid">
      <?php if (!empty($galeri)): ?>
        <?php foreach ($galeri as $gal): ?>
          <a href="<?= base_url('Galeri/detail/' . $gal->id_galeri) ?>" class="guru-card">
            <?php if (!empty($gal->foto) && file_exists('./upload/galeri/' . $gal->foto)): ?>
              <img class="thumb" src="<?= base_url('upload/galeri/' . $gal->foto) ?>" alt="<?= html_escape($gal->judul_kegiatan) ?>">
            <?php else: ?>
              <div class="thumb-placeholder"><i class="bi bi-images"></i></div>
            <?php endif; ?>
            <div class="body">
              <h3><?= html_escape($gal->judul_kegiatan) ?></h3>
              <div class="jabatan"><?= !empty($gal->lokasi) ? html_escape($gal->lokasi) : '-' ?></div>
              <div class="meta">
                <?php if (!empty($gal->tanggal)): ?><span><i class="bi bi-calendar-event"></i> <?= html_escape(date('d M Y', strtotime($gal->tanggal))) ?></span><?php endif; ?>
              </div>
        ...
        <?php endforeach; ?>
      <?php else: ?>
        <p class="empty-state">Belum ada data galeri kegiatan.</p>
      <?php endif; ?>
    </div>
